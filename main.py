...
tuner = kt.BayesianOptimization(model_builder, objective=kt.Objective('val_rmse', direction='min'), max_trials=100, executions_per_trial=2)
tuner.search(x=x_train, y=y_train, epochs=100, batch_size=256, validation_data=(x_test, y_test), callbacks=[es])
tuned_model = tuner.get_best_models(1)[0]
tuned_model.save("saved_model/best_model")